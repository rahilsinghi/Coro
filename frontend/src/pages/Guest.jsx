  ...
  const toggleMic = async () => {
    ...
    try {
      ...
      const interval = setInterval(() => {
        ...
        const now = Date.now()
        if (instant - smoothedAvg > 0.30 && instant > 0.25) {
          if (!clapTimestamps.length || now - clapTimestamps[clapTimestamps.length - 1] > 250) {
        ...
        const gatedVolume = instant > 0.20 ? instant : 0
